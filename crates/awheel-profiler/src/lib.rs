// NOTE: This is directly taken from puffin (as a poc)
...
impl ThreadProfiler {
    ...
    /// so you'll want to call `awheel_profiler::ThreadProfiler::initialize(my_timing_function, awheel_profiler::global_reporter);`.
    ...
    pub fn end_scope(&mut self, start_offset: usize) {
        ...
        if self.depth > 0 {
            ...
            eprintln!("awheel_profiler ERROR: Mismatched scope begin/end calls");
        }
    ...
    pub fn new_frame(&mut self) {
        ...
        let new_frame =
            match FrameData::new(current_frame_index, std::mem::take(&mut self.current_frame)) {
                ...
                Err(err) => {
                    eprintln!("awheel_profiler ERROR: Bad frame: {err:?}");
                    return;
                }
            };
...
/// Created by the `awheel_profiler::profile*!(...)` macros.
...
///     awheel_profiler::profile_function!();
///     /* … */
///     # let image = Image {};
///     image
/// }
...
/// # struct Image {};
/// fn load_image(path: &str) -> Image {
///     awheel_profiler::profile_function!(path);
///     /* … */
///     # let image = Image {};
///     image
/// }
